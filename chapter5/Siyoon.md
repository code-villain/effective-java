...
# item33) 타입 안전 이종 컨테이너

## 정의
### 단일원소 컨테이너

하나의 컨테이너에서 매개변수화 할 수 있는 타입이 제한적인 (일반적인) 제네릭 타입.
매개변수화되는 대상은 (원소가 아닌) 컨테이너 자신이다. (제네릭 타입이 공변이 아니기에 유연함이 떨어지고 그래서 와일드카드나 제네릭 메서드롤 사용해야 했다.)

### 타입 안전 이종 컨테이너(type safe heterogeneous container)

'이종' (heterogeneous)라는 말이 핵심이다. 다른 '종'의 원소들을 담을 수 있는 컨테이너 라는 뜻.
이종 컨테이너를 만들기 위해서는
1. 컨테이너 대신 class 리터럴인 Class<T>로 된 'key'를 매개변수화 한다.
2. 이제 컨테이너를 Map 처럼 다룬다. 'key'를 이용해서 put(), get()하는 행위를 한다.

'key'를 Class<T>로 받게 되니 특정 타입에 종속되지 않고, 여러 다른 '종'의 타입을 담을 수 있게 된다.
